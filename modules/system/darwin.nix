...
{ config, pkgs, ... }:
{
  ...
  system.defaults.NSGlobalDomain = {
    ...
    # Activating an app (e.g. an `open -a` hotkey) switches to the macOS desktop
    # that holds its windows. AeroSpace (aerospace.nix) handles app-to-workspace
    # binding; this only matters if you add extra macOS desktops.
    AppleSpacesSwitchOnActivate = true;
  };

  system.defaults.dock = {
    # Keep macOS desktops in a fixed order instead of reordering them by recent
    # use.
    mru-spaces = false;

    # Dock at the bottom, hidden until the pointer reaches the bottom edge.
    # macOS shows one Dock at a time: with "Displays have separate Spaces" on
    # (the default), it appears on whichever display you move the pointer to
    # the bottom of. AeroSpace also recommends a bottom, auto-hiding Dock.
    orientation = "bottom";
    autohide = true;
  };
}
